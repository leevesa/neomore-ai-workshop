...
As a Java developer, you should build the backend API using Spring Boot so that it satisfies the product requirements and the shared OpenAPI contract.
...
- [Build Spring Boot API App](#build-spring-boot-api-app)
...
### Build Spring Boot API App

1. Make sure that you're using GitHub Copilot Agent Mode with the model of `Claude Sonnet 4` or `GPT-4.1`.
1. Make sure that the `context7` MCP server is up and running.
1. Add [`product-requirements.md`](../product-requirements.md) and [`openapi.yaml`](../openapi.yaml) to GitHub Copilot.
1. Use prompt like below to build the Spring Boot API.

    ```text
    I'd like to build a Spring Boot API app. Follow the instructions below.
...
1. Click the ![the "keep" button image](https://img.shields.io/badge/keep-blue) button of GitHub Copilot to take the changes.
1. Once implementation is over, use prompt like below to verify the result.
...
OK. You've completed the "Java" step. Let's move onto [STEP 02: Containerization](./05-containerization.md).
