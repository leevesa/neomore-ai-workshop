...
class WorkshopHubService extends cds.ApplicationService {
  ...
  async doHeartbeat(req) {
    ...
    await this.guard(req, () => hub.sendHeartbeat(this.connection.participantId, hubPassword(req)))
  }
...
function chatFields(data) {
  ...
  if (data.replyToEventId !== null && data.replyToEventId !== undefined) {
    fields.metadata = { replyToEventId: data.replyToEventId }
  }
  return fields
}
