...
service WorkshopHubService @(path: '/workshop-hub') {
    ...
    /** Send a chat message, optionally replying to an existing feed event. */
    action sendChatMessage(message : String, replyToEventId : Integer64) returns Feed;

    /** Send a participant-attributed heartbeat. */
    action heartbeat();
